...
- **Main session ONLY** — never load in group chats or shared contexts (security)
- Curated essence, not raw logs — distill from daily files over time

### Write Discipline
- "Mental notes" don't survive restarts. WRITE IT DOWN.
- After every task: log to `memory/YYYY-MM-DD.md`
- Mistake or lesson: append one-liner to `learnings/LEARNINGS.md`
- Significant long-term context: update `MEMORY.md` (during heartbeats, not mid-task)
...
**Rule:** speak when you add value, stay silent otherwise. quality > quantity. one response beats three fragments. participate, don't dominate.
...
**Quiet hours:** 23:00-08:00 tallinn unless urgent.

**Proactive work (idle time):** organize memory, check projects, update docs, commit/push, compress logs into MEMORY.md.

**Memory maintenance:** every few days, review recent daily logs → distill into MEMORY.md → remove outdated entries. Daily files are raw notes; MEMORY.md is curated wisdom.
