#!/usr/bin/env node
/**
 * HYPE 5m Slope Bot v2 - WITH COMMITMENT FILTER
 * Strategy: 200 EMA + crossover + 0.01% slope + FULL COMMITMENT (10/10 closes above/below EMA)
 * 5% profit / 2% stoploss
 */
...
(async () => {
  ...
  for (let i = 210; i < closes.length; i++) {
    ...
    if (!position) {
      // Entry: crossover + slope + COMMITMENT
      const slopeOk = Math.abs(slope) > 0.01;
      const closesAbove = countClosesAboveEMA(closes, emaVals, i, 10);
      
      // LONG: price must cross above, slope positive, AND last 10 must be 9-10 above EMA
      if (prevClose <= prevEma && close > emaVal && slope > 0 && slopeOk && closesAbove >= 9) {
        position = {
          ...
          commitment: closesAbove,
        };
      }
      // SHORT: price must cross below, slope negative, AND last 10 must be 1-2 above EMA (9-10 below)
  ...
  console.log('═══════════════════════════════════════════════════════════');
  console.log(`HYPE 30d | 200 EMA + 0.01% slope + COMMITMENT FILTER + 5%/-2%`);
  console.log('═══════════════════════════════════════════════════════════\n');
  
  console.log(`Trades: ${trades.length} (vs 12 without filter)`);
  ...
  if (trades.length > 0) {
    console.log(`# | Type  | Entry    | Exit     | P&L %  | Slope% | Commit | Reason | Bars`);
    console.log('──────────────────────────────────────────────────────────────────────────');
    trades.forEach((t, i) => {
      const icon = parseFloat(t.pnl) > 0 ? '✅' : '❌';
      const typeStr = t.type === 'LONG' ? 'LONG ' : 'SHORT';
      console.log(`${String(i+1).padEnd(2)}| ${typeStr}| ${t.entry.padEnd(8)}| ${t.exit.padEnd(8)}| ${icon} ${t.pnl.padEnd(5)}| ${t.slope.padEnd(5)}| ${t.commitment}/10   | ${t.reason.padEnd(6)}| ${t.bars}`);
    });
  }
  ...
  if (trades.length === 0) {
    console.log('❌ NO TRADES: Commitment filter too strict (need 9-10/10 aligned closes)\n');
  ...
  console.log('Without filter: 12 trades, 25% WR, -8.84% P&L');
  console.log(`With filter:    ${trades.length} trades, ${wr}% WR, ${pnlTotal.toFixed(2)}% P&L`);
  console.log(`Trade reduction: ${(12 - trades.length)} eliminated (false entries filtered)\n`);
})();
