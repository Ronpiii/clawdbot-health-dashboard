#!/usr/bin/env node
/**
 * HYPE 5m Slope Bot v3 - STRICT COMMITMENT (10/10 or 0/10 only)
 */
...
(async () => {
  ...
  for (let i = 210; i < closes.length; i++) {
    ...
    if (!position) {
      ...
      // STRICT: only 10/10 or 0/10, no 9/10
      if (prevClose <= prevEma && close > emaVal && slope > 0 && slopeOk && closesAbove === 10) {
        position = {
          type: 'LONG',
          entry: close,
          bar: i,
      ...
      } else if (prevClose >= prevEma && close < emaVal && slope < 0 && slopeOk && closesAbove === 0) {
        position = {
          type: 'SHORT',
          entry: close,
          bar: i,
  ...
  console.log('═══════════════════════════════════════════════════════════');
  console.log(`HYPE 30d | 200 EMA + 0.01% slope + STRICT COMMITMENT (10/10 only)`);
  console.log('═══════════════════════════════════════════════════════════\n');
  
  console.log(`Trades: ${trades.length} (vs 12 without filter, vs 8 with loose filter)`);
  ...
  if (trades.length === 0) {
    console.log('❌ NO TRADES: Strict commitment filter eliminated all trades\n');
  ...
  console.log('No filter:      12 trades, 25% WR, -8.84% P&L');
  console.log('Loose (9-10):   8 trades, 25% WR, -5.82% P&L (+3.02% improvement)');
  console.log(`Strict (10/10): ${trades.length} trades, ${wr}% WR, ${pnlTotal.toFixed(2)}% P&L\n`);
})();
