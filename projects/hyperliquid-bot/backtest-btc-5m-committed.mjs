#!/usr/bin/env node
/**
 * BTC 5m Slope Bot v2 - WITH COMMITMENT FILTER
 * Strategy: 200 EMA + crossover + 0.01% slope + COMMITMENT (9-10 aligned closes)
 * 5% profit / 2% stoploss
 */
...
(async () => {
  ...
  for (let i = 210; i < closes.length; i++) {
    ...
    if (!position) {
      ...
      // LONG: crossover + positive slope + 9-10 closes above EMA
      ...
      // SHORT: crossover + negative slope + 0-1 closes above EMA (9-10 below)
  ...
  console.log('═══════════════════════════════════════════════════════════');
  console.log(`BTC 30d | 200 EMA + 0.01% slope + COMMITMENT FILTER + 5%/-2%`);
  console.log('═══════════════════════════════════════════════════════════\n');
  ...
  if (trades.length > 0) {
    ...
    trades.forEach((t, i) => {
      ...
      console.log(`${String(i+1).padEnd(2)}| ${typeStr}| $${t.entry.padEnd(6)}| $${t.exit.padEnd(6)}| ${icon} ${t.pnl.padEnd(5)}| ${t.slope.padEnd(5)}| ${t.commitment}/10   | ${t.reason.padEnd(6)}| ${t.bars}`);
    });
  }
  ...
  console.log('BTC baseline (no commitment filter): +12.7% (from previous backtest)');
  console.log(`BTC with commitment filter:         ${pnlTotal > 0 ? '+' : ''}${pnlTotal.toFixed(2)}% (${trades.length} trades)\n`);
})();
