 ...
 * Entry: crossover + EMA slope is rising/falling (not flat)
 * Exit: +2% profit OR 5% stoploss (whichever hits first)
 */
...
console.log(`\n📊 ${SYMBOL.replace('USDT', '')} ${DAYS}d | 200 EMA slope filter + ${PROFIT_TARGET}% profit take\n`);
...
function getEmaSlope(emaVals, barIndex, lookback = 10) {
  ...
  return ((recent - past) / past) * 100; // % change
}
...
function backtest(candles) {
  ...
  for (let i = 200; i < closes.length; i++) {
    ...
    if (!position) {
      const hasSlope = Math.abs(slope) > 0.1; // EMA moving, not flat
  ...
  console.log(`═══════════════════════════════════════════════════════`);
  console.log(`200 EMA | Slope Filter + ${PROFIT_TARGET}% Profit Take`);
  console.log(`═══════════════════════════════════════════════════════\n`);
  
  console.log(`Trades: ${trades.length} (rejected: ${rejectedEntries} flat EMA entries)`);
  ...
  trades.forEach((t, i) => {
    ...
    console.log(`${i+1} | ${t.type.padEnd(5)}| $${t.entry.padEnd(7)}| $${t.exit.padEnd(7)}| ${e} ${t.pnl.padEnd(5)}| ${t.slope.padEnd(5)}| ${t.reason.padEnd(6)}| ${t.bars}`);
  });
  ...
  if (trades.length === 0) {
    ...
    console.log(`⚠️  MARGINAL: Low win rate but still profitable\n`);
  } else {
