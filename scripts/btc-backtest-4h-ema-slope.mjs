#!/usr/bin/env node
/**
 * BTC 4H 200 EMA Strategy Backtest - WITH SLOPE FILTER
 * 
 * Rules:
 * - LONG: 4H close crosses above 200 EMA AND EMA is rising
 * - SHORT: 4H close crosses below 200 EMA AND EMA is falling
 * - Exit: candle closes on opposite side of 200 EMA
 * 
 * Slope = compare current EMA to EMA N candles ago
 */
...
// --- Simulate Trades WITH SLOPE FILTER ---
function simulateTrades(candles, ema200, leverage = 3, slopeLookback = 12) {
  const trades = [];
  let position = null;
  ...
  for (let i = 201; i < candles.length; i++) {
    ...
    const emaLookback = ema200[i - slopeLookback];
    
    const aboveEMA = candle.close > ema;
    const prevAboveEMA = prevCandle.close > prevEma;
    
    // Slope: is EMA rising or falling over lookback period?
    const emaRising = ema > emaLookback;
    const emaFalling = ema < emaLookback;
    ...
    // Check for entry (cross + slope confirmation)
    if (!position) {
      // Crossed above EMA -> check for LONG
      ...
      // Crossed below EMA -> check for SHORT
...
function calculateMetrics(trades, initialCapital = 10000, leverage = 3, riskPercent = 0.025) {
  ...
  for (const trade of closedTrades) {
    ...
    const pnl = riskAmount * (trade.pnlLeveraged / 0.08);
  ...
  const avgHoldHours = closedTrades.length > 0 
    ? closedTrades.reduce((sum, t) => sum + t.hoursHeld, 0) / closedTrades.length
    : 0;
...
async function main() {
  ...
  const leverage = args.includes('--leverage')
    ...
    : 3;
  const slopeLookback = args.includes('--slope')
    ? parseInt(args[args.indexOf('--slope') + 1])
    : 12; // 12 x 4h = 48 hours = 2 days
  ...
  const { trades, skippedNoSlope } = simulateTrades(candles, ema200, leverage, slopeLookback);
  ...
  console.log('═══════════════════════════════════════════════════════════════');
  console.log('  4H 200 EMA STRATEGY + SLOPE FILTER');
  console.log('═══════════════════════════════════════════════════════════════');
  console.log(`  period:        ${startDate} → ${endDate}`);
  console.log(`  candles:       ${candles.length} (4H)`);
  console.log(`  leverage:      ${leverage}x`);
  console.log(`  slope lookback: ${slopeLookback} candles (${slopeLookback * 4}h)`);
  console.log(`  entry:         cross EMA + slope confirms direction`);
  console.log(`  exit:          close crosses back`);
  console.log('');
  
  console.log('  FILTER IMPACT');
  console.log('  ─────────────────────────────────────────────────────────');
  console.log(`  signals skipped: ${skippedNoSlope} (no slope confirmation)`);
  console.log(`  trades taken:    ${metrics.closedTrades + metrics.openTrades}`);
  console.log('');
  ...
  console.log('  VERDICT');
  console.log('  ─────────────────────────────────────────────────────────');
  if (metrics.ev > 0.01 && metrics.profitFactor > 1.2) {
    console.log('  ✓ TRADEABLE — positive EV with good profit factor');
    console.log(`  → recommended risk: ${Math.min(metrics.kelly * 25, 5).toFixed(1)}% per trade`);
  } else if (metrics.ev > 0) {
    console.log('  ⚠ MARGINAL — positive EV but needs more edge');
  } else {
  ...
  const resultsFile = join(WORKSPACE, '.cache', 'btc-4h-ema-slope-results.json');
  ...
  console.log(`saved to .cache/btc-4h-ema-slope-results.json`);
}
