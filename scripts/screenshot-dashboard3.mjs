...
// Mobile screenshots — each page
for (const p of pages) {
  await page.setViewport({ width: 390, height: 844, deviceScaleFactor: 2 });
  await page.goto(`https://anivia.vercel.app${p.path}`, { waitUntil: 'networkidle2', timeout: 20000 });
  await new Promise(r => setTimeout(r, 3000));
  await page.screenshot({ path: `${OUT}/dash-${p.name}-mobile.png`, fullPage: false });
  console.log(`mobile: ${p.name}`);
}

await browser.close();

// Build hero mockups with matching mobile
...
for (const p of pages) {
  ...
  const mobileB64 = readFileSync(`${OUT}/dash-${p.name}-mobile.png`).toString('base64');
