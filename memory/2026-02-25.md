...
## ventok.eu audit report received (19:36 UTC)
### critical
1. footer CSS override too broad — `footer` selector matches all footers
2. React hydration error #418 on every page — caused by CSS/JS modifications to static Next.js export
3. missing favicon.ico (404)

### significant
4. logo-white.png swap via CSS `content:` is fragile
5. desktop nav links /services#section, mobile nav links /services/section — inconsistent
6. all buttons type="submit" instead of type="button"

### minor
7. form labels missing `for` attribute
8. missing og:image and canonical URL tags
9. meta description not page-specific
